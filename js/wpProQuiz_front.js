	...
	$.wpProQuizFront = function(element, options) {
		...
		function QuestionTimer() {
			...
			this.questionStop = function() {
				...
				return results[currentQuestionId].time;
		 ...
		 * @memberOf checker
		 */
		var checker = function(name, data, $question, $questionList, qtime) {
			var correct = true;
			var points = 0;
...
//						if(i == $stringItem.data('pos')) {
			...
			if(!isDiffPoints && correct) {
				...
				if(qtime <= 5){
					...
					points = Number(data.points) + Number(qextra);
			}

			return {c: correct, p: points, s: statistcAnswerData, e: Number(qextra)};
		}; 
...
//					plugin.methode.showQuizSummary();
			...
			setCategoryOverview: function() {
				...
				$e.find('.wpProQuiz_catOverview li').each(function() {
					...
					console.log(catResults[catId]);
					console.log(config.catPoints[catId]);
...
//				$e.find('.wpProQuiz_cloze input').val('');
			...
			checkQuestion: function(list, endCheck) {
				...
				list.each(function() {
					...
					qtime = questionTimer.questionStop();
					...
					var result = checker(name, data, $this, $questionList, qtime);
					...
					config.catPoints[data.catId] += result.e;

					config.globalPoints += result.e;
