...
// 		}
	...
	public function save($quiz = null) {
		...
		// setcookie('Save_Quiz', 'this-will-save6', time() + 60*60*24*300, empty($url['path']) ? '/' : $url['path']);
		// 
		...
		// 	$lockMapper->insert($lo//)setcookie('Save_Quiz', 'this-will-save7', time() + 60*60*24*300, empty($url['path']) ? '/' : $url['path']);
